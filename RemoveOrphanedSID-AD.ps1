...
$Forest = Get-ADRootDSE -Server 10.2.0.103
$Domain = (Get-ADDomain -Server 10.2.0.103).distinguishedname
...
function RemovePerms($fold) {
  ...
  # Write-Host $x.access
  if ($mod) { Set-ACL -aclobject $x -path AD:$f; Write-Host "Orphaned SID removed on $fname" -ForegroundColor Red }
}
